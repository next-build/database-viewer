<template>
    <div v-if="isMobileDevice()">
        ...
        <Dialog as="div" class="relative z-40" @close="() => emit('updateSidebarOpen', sidebarOpen)">
                    ...
                    <DialogPanel class="relative mr-16 flex w-full max-w-xs flex-1">
                        <TransitionChild as="template" enter="ease-in-out duration-300" enter-from="opacity-0"
                            ...
                            <div class="absolute left-full top-0 flex w-16 justify-center pt-5">
                                <button type="button" class="-m-2.5 p-2.5" @click="() => emit('updateSidebarOpen', sidebarOpen)">
                        ...
                        <div class="flex grow flex-col gap-y-5  bg-gray-900 px-6 pb-4">
                            ...
                            <nav class="flex flex-col gap-6">
                                    ...
                                    <div class="mt-2">
                                        <div class="relative rounded-md shadow-sm">
                                            <select id="column" name="column"
                                                ...
                                                <option class="text-md text-gray-900"
                                                    v-for="(database, index) in databases" :key="index">
                                                    {{ database }}
                                                </option>
                                            </select>
                                            <div
                                                class="text-white pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                    stroke-width="1.5" stroke="currentColor" class="w-4 h-4 rotate-90">
                                ...
                                <div v-if="selectedDatabase" class="-mx-2">
                                    ...
                                    <input type="text" name="table" id="table"
                                        ...
                                        placeholder="Search Table" @input="(e) => {
                                            tableSearch = e.target.value
                                        }" :autocomplete="false" />

                                    <div class="h-screen overflow-auto navigation-scroller mt-2 pb-32">
                                        <ul role="list" class="flex flex-1 flex-col mt-2 mr-2 gap-y-7 mb-44">
                                            <li>
                                                <ul role="list" class="space-y-4">
                                                    <li v-for="(table, index) in tableSearch ? tables.filter(x => x.toLowerCase().includes(tableSearch.toLowerCase())) : tables"
                                                        :key="index">
                                                        <a @click="() => {
                                                            selectedTable = table;
    ...
    <div v-else-if="sidebarOpen" class="hidden lg:fixed lg:inset-y-0 lg:z-40 lg:flex lg:w-72 lg:flex-col">
        <!-- Sidebar component, swap this element with another sidebar if you like -->
        <div class="flex grow flex-col gap-y-5  bg-gray-900 px-6 pb-4">
            <div class="flex h-16 shrink-0 items-center -mx-2">
                <span class="text-2xl font-medium text-white">Database Viewer</span>
                    ...
                    <div class="mt-2">
                        <div class="relative rounded-md shadow-sm">
                            <select id="column" name="column"
                                ...
                                <option class="text-md text-gray-900" v-for="(database, index) in databases"
                                    :key="index">
                                    {{ database }}
                                </option>
                            </select>
...
const emit = defineEmits(['selectTable', 'updateSidebarOpen']);
...
function isMobileDevice() {
    ...
    return /android|iPad|iPhone|iPod|blackberry|mini|windows\sce|palm/i.test(userAgent);
}
</script>
